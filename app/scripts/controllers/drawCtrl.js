    ...
    [ '$scope', 'hlp', '$window', '$timeout', 'MixpanelService', //
        function ($scope, hlp, $window, $timeout, MixpanelService) { //
            ...
            function isInRange(position) {
                return 1 <= position && position <= allItems.length;
            }

            $scope.isValidRange = function() {
                var inputFrom = parseInt($scope.data_from, 10);
                var inputTo = parseInt($scope.data_to, 10);

                if (_.isNaN(inputFrom) || _.isNaN(inputTo)) {
                    return false;
                }

                if (!isInRange(inputFrom) || !isInRange(inputTo)) {
                    return false;
                }

                return inputTo - inputFrom >= 5;
            };
            ...
            $scope.$watch('data_from', function() {
                ...
                $scope.firstSymbol = getKey(input);
            });
